...
class GTFSClient:
    def __init__(
        ...
        refreshed, new_mtime = refresh_feed.update_local_file_from_url_v1(
            last_mtime, feed_name, feed_url
        )
    ...
    def _read_feed(
        self, path: gk.Path, dist_units: str, stop_codes: list[str]
    ) -> gk.Feed:
        """
        ...
        with zipfile.ZipFile(path) as z:
            ...
            wanted_stop_ids = feed_dict.get("stops")[
                feed_dict.get("stops")["stop_code"].isin(self.stop_codes)
            ]["stop_id"]
            with z.open("stop_times.txt") as f:
                ...
                df = pd.concat(
                    [
                        chunk[chunk["stop_id"].isin(wanted_stop_ids)]
            ...
            if not df.empty:
                ...
                df["arrival_time"] = df.apply(
                    lambda row: row["arrival_time"]
                    if len(row["arrival_time"]) == 8
                    else "0" + row["arrival_time"],
                    axis=1,
                )
                gc.collect()
                df["departure_time"] = df.apply(
                    lambda row: row["departure_time"]
                    if len(row["departure_time"]) == 8
                    else "0" + row["departure_time"],
                    axis=1,
                )
                gc.collect()
    ...
    def __service_ids_active_at(self, when: datetime) -> pd.core.frame.DataFrame:
        ...
        active_calendars = self.feed.calendar.query(
            "start_date <= @todays_date and end_date >= @todays_date and {} == 1".format(
                todays_weekday
            )
        )
    ...
    def __current_calendars(self) -> pd.core.frame.DataFrame:
        ...
        if active_calendars.empty:
            print(
                "The concatenation of today and tomorrow's calendars is empty. This should not happen."
            )
    ...
    def __trip_ids_for_service_ids(
    ...
    def __next_n_buses(
        self, trip_ids: pd.core.series.Series, n: int
    ) -> pd.core.frame.DataFrame:
        now = datetime.datetime.now()
        current_time = now.strftime("%H:%M:%S")
        next_stops = self.feed.stop_times[
            self.feed.stop_times["stop_id"].isin(self.stop_ids)
    ...
    def __join_data(
        self, next_buses: pd.core.frame.DataFrame
    ) -> pd.core.frame.DataFrame:
        """
        Enriches the stop data with the information from other dataframes in the feed
        """
        joined_data = (
            next_buses.join(self.feed.trips.set_index("trip_id"), on="trip_id")
    ...
    def __poll_gtfsr_deltas(self) -> list[map, set]:
        try:
            ...
            relevant_trips = self.feed.trips[
                self.feed.trips["service_id"].isin(relevant_service_ids)
            ]
                ...
                try:
                    ...
                    if trip_action == "SCHEDULED":
                        for u in e.get("trip_update", {}).get("stop_time_update", []):
                            delay = u.get("arrival", u.get("departure", {})).get(
                                "delay", 0
                            )
                        ...
                        for stop_time_update in e.get("trip_update").get(
                            "stop_time_update", []
                        ):
                            if stop_time_update.get("stop_id", "") in wanted_stop_ids:
                                arrival_time = int(
                                    (
                                        stop_time_update.get(
                                            "arrival",
                                            stop_time_update.get("departure", {}),
                                        ).get("time", 0)
                                    )
                                )
                                ...
                                new_arrival = ArrivalTime(
                                    ...
                                    route_id=self.feed.routes[
                                        self.feed.routes["route_id"] == route_id
                                    ]["route_short_name"].item(),
                                    route_type=self.feed.routes[
                                        self.feed.routes["route_id"] == route_id
                                    ]["route_type"].item(),
                                    destination=self.__lookup_headsign_by_route(
    ...
    def refresh(self):
        ...
        for index, bus in buses.iterrows():
            if not bus["trip_id"] in self.canceled_trips:
                ...
                if delta != 0:
                    print(
                        "Delta for route {} stop {} is {}".format(
                            bus["route_short_name"], bus["stop_id"], delta
                        )
                    )

                arrival = ArrivalTime(
                    stop_id=bus["stop_code"],
